...
Novata lets users organize their notes as nested pages (similar to Notion), mark pages as favorites, and search across their content. This repository is the API-only backend that powers the app — it has no Blade/frontend views of its own and is meant to be consumed by a separate client at the URL configured in `APP_FRONTEND_URL`.
## Tech Stack
